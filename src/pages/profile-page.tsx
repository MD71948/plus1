  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: 'var(--bg)' }}>
      ...
      <div className="relative"
        style={{ background: 'linear-gradient(135deg, #3B0764 0%, #5B21B6 45%, #7C3AED 75%, #9F67FF 100%)', paddingBottom: '60px' }}>
        {/* Decorative circles */}
        ...
        <div style={{ position: 'absolute', top: 20, right: 40, width: 80, height: 80, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', pointerEvents: 'none' }} />
        ...
        <div className="relative flex items-center justify-between px-5 pt-14 pb-3">
          <div>
            <span className="text-white/60 text-[10px] font-black uppercase tracking-widest block mb-1">Mein Profil</span>
            <h1 className="text-2xl font-black text-white leading-tight">
              {profile.name}
              {profile.age && <span className="text-white/50 font-normal text-lg ml-2">{profile.age}</span>}
            </h1>
            {(profile.city || profile.district) && (
              <div className="flex items-center gap-1 mt-1 text-white/60 text-xs font-semibold">
          ...
          <button onClick={onSignOut} className="text-white/60 text-sm font-semibold hover:text-white transition-colors">
            Ausloggen
          </button>
        </div>
      </div>

      {/* Avatar + edit overlapping header */}
      <div className="relative px-5 -mt-14">
        <div className="flex items-end justify-between mb-5">
          <div className="relative">
            <div className="w-28 h-28 rounded-3xl overflow-hidden"
              style={{ border: '3px solid white', boxShadow: '0 8px 24px rgba(0,0,0,0.18)' }}>
              {profile.avatar_url ? (
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-2xl text-sm font-bold text-gray-700 bg-white border border-gray-200 transition-all"
        ...
        {/* Bio */}
        {profile.bio && (
          <p className="text-sm leading-relaxed text-gray-600 mb-5">{profile.bio}</p>
        )}

        {/* Stats */}
        <div className="grid grid-cols-4 gap-2 mb-5">
          {[
            { value: profile.activities_count ?? 0, label: 'Activities', icon: '⚡' },
            { value: followerCount, label: 'Follower', icon: '👥' },
            { value: followingCount, label: 'Following', icon: '➡️' },
            { value: crew.length, label: 'Crew', icon: '🤝' },
          ].map(stat => (
        ...
        {/* Interessen */}
        {allTags.length > 0 && (
          <div className="bg-white rounded-3xl p-4 mb-3" style={{ border: '1px solid var(--border)', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            <p className="text-[10px] font-black uppercase tracking-widest mb-3 text-gray-400">🎯 Interessen</p>
            <div className="flex flex-wrap gap-2">
              {allTags.map(tag => (
                <span key={tag} className="px-3 py-1.5 rounded-full text-xs font-bold"
                  style={{ background: 'linear-gradient(135deg, #EDE9FE, #F5F3FF)', color: '#7C3AED', border: '1px solid #DDD6FE' }}>
                  {tag}
                </span>
              ))}
            </div>
        ...
        {profile.languages?.length > 0 && (
          <div className="bg-white rounded-3xl p-4 mb-3" style={{ border: '1px solid var(--border)', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            <p className="text-[10px] font-black uppercase tracking-widest mb-3 text-gray-400">🌍 Sprachen</p>
            <div className="flex flex-wrap gap-2">
              {profile.languages.map(lang => (
                <span key={lang} className="px-3 py-1.5 rounded-full text-xs font-bold text-gray-600 bg-gray-50 border border-gray-200">
                  {lang}
                </span>
              ))}
            </div>
